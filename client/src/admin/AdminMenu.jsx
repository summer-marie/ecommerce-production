...
const AdminMenu = () => {
  ...
  const { active: activePizzas, inactive: inactivePizzas } = getSeparatedBuilders();
    ...
    <>
      <div className="px-4">
        ...
        <div className="mb-10 mx-auto w-full px-6 py-2 sm:px-6 lg:max-w-7xl lg:px-8">
          <h3 className="text-xl font-bold text-gray-800 mb-4">Active Menu Items</h3>
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className={`absolute z-10 bottom-1 left-1 sm:bottom-2 sm:left-2 font-medium rounded-lg shadow-lg text-xs px-2 py-1 sm:text-sm sm:px-5 sm:py-2.5 text-center bg-gradient-to-t focus:ring-4 focus:outline-none whitespace-nowrap transform origin-bottom-left transition-all sm:scale-100 max-[640px]:scale-90 max-[420px]:scale-75
                            ...
                            ${togglingId === (builder.id || builder._id) ? 'opacity-60 cursor-not-allowed' : 'hover:bg-gradient-to-br cursor-pointer'}`}
                        >
                          {togglingId === (builder.id || builder._id) ? 'Updating...' : 'Deactivate'}
                        </button>
                      </div>
        ...
        {inactivePizzas.length > 0 && (
          <div className="mb-10 mx-auto w-full px-6 py-2 sm:px-6 lg:max-w-7xl lg:px-8">
            <hr className="my-6 sm:mx-auto lg:my-8 border-gray-700" />
            <h3 className="text-xl font-bold text-gray-600 mb-4">Deactivated Pizzas</h3>
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        className={`absolute z-10 top-1 right-1 sm:top-2 sm:right-2 font-medium rounded-lg shadow-lg text-xs px-2 py-1 sm:text-sm sm:px-5 sm:py-2.5 text-center bg-gradient-to-t focus:ring-4 focus:outline-none whitespace-nowrap transform origin-top-right transition-transform sm:scale-100 max-[640px]:scale-90 max-[420px]:scale-75
                          ...
                          ${togglingId === (builder.id || builder._id) ? 'opacity-60 cursor-not-allowed' : 'hover:bg-gradient-to-br cursor-pointer'}`}
                      >
                        {togglingId === (builder.id || builder._id) ? 'Updating...' : 'Activate'}
                      </button>
                    </div>
                    <div className="p-3 flex-1 flex flex-col">
                      <p className="text-gray-700">
                        <strong>Name: {builder?.pizzaName || "Unnamed"}</strong>
                        <span className="ml-2 text-xs bg-gray-500 text-white px-2 py-1 rounded">DRAFT</span>
                      </p>
