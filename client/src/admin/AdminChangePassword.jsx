  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      if (changeEmail.fulfilled.match(action)) {
        ...
        setEmailResult({
          ok: false,
          message: action.payload || "Email update failed",
        });
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (changePassword.fulfilled.match(action)) {
        ...
        setResult({
          ok: true,
          message: action.payload?.requireRelogin
        ...
        setResult({
          ok: false,
          message: action.payload || "Change password failed",
        });
      }
    } catch (err) {
      ...
      <div className="w-full max-w-lg">
        <div className="rounded-2xl bg-white/90 shadow-xl ring-1 ring-slate-200 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-7 space-y-4">
            <div>
              <label className="block text-xs font-semibold tracking-wide uppercase text-slate-600 mb-1">
                Current password
              </label>
