...
const orderService = {
  ...
  createOrder: async (order) => {
    return await axios.post(`${API_BASE}/orders`, order);
  },
  ...
  createOrderWithPayment: async (orderData, paymentToken) => {
    return await axios.post(`${API_BASE}/orders/with-payment`, {
      ...orderData,
      paymentToken,
    });
  },
  ...
  orderGetOpen: async () => {
    return await axios.get(`${API_BASE}/orders/open`);
  },

  // Get only archived orders
  orderGetArchived: async () => {
    return await axios.get(`${API_BASE}/orders/archived`);
  },

  // Update order status
  orderUpdateStatus: async (order) => {
    const { id, status } = order;
    return await axios.put(`${API_BASE}/orders/open/${id}`, { status: status });
  },

  // Archive order
  orderArchiveOne: async (id) => {
    return await axios.put(`${API_BASE}/orders/archive/${id}`, {
      isArchived: true,
    });
  },
};
