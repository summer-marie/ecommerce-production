...
const shouldSave = (process.env.SAVE_CONTACTS || "true").toLowerCase() === "true";
...
const messageCreate = async (req, res) => {
  try {
    ...
    const from = process.env.CONTACT_FROM_EMAIL || process.env.SENDGRID_FROM_EMAIL || "noreply@yoursite.com";
    ...
    if (process.env.SENDGRID_API_KEY) {
      try {
        ...
        mailResult = { messageId: result[0]?.headers?.['x-message-id'] || 'sent' };
        console.log("Contact email sent via SendGrid:", mailResult.messageId);
      } catch (err) {
        console.error("Failed to send contact email via SendGrid:", err?.message || err);
      }
    } else {
    ...
    if (!mailResult && !saved) {
      return res.status(500).json({ success: false, error: "Failed to deliver or save message" });
    }
    ...
    return res.status(500).json({ success: false, error: "Server error while processing message" });
  }
};
